...
civicus <- read_csv(file.path(here(), "data", "civicus_monitor_2017.csv"),
                    na = "Null") %>%
...
plot_civicus_population %T>% 
  print() %T>%
  ggsave(., filename = file.path(here(), "output", "civicus-population.pdf"),
         width = 4.5, height = 2, units = "in", device = cairo_pdf) %>% 
  ggsave(., filename = file.path(here(), "output", "civicus-population.png"),
...
plot_civicus_map %T>% 
  print() %T>%
  ggsave(., filename = file.path(here(), "output", "civicus-map.pdf"),
         width = 5.5, height = 3, units = "in", device = cairo_pdf) %>% 
  ggsave(., filename = file.path(here(), "output", "civicus-map.png"),
...
amera_budget <- read_csv(file.path(here(), "data", "amera_budget.csv")) %>% 
...
plot_budget_staff_amera %T>% 
  print() %T>%
  ggsave(., filename = file.path(here(), "output", "amera-budget-staff.pdf"),
         width = 4.5, height = 3.5, units = "in", device = cairo_pdf) %>% 
  ggsave(., filename = file.path(here(), "output", "amera-budget-staff.png"),
...
amera_partners <- read_csv(file.path(here(), "data", "amera_partners.csv")) %>% 
  # Convert lat/long to actual geometry object and convert to correct projection
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  st_transform(projection) %>% 
  # Extract converted lat/long for use with geom_label_repel
...
plot_amera_offices %T>% 
  print() %T>%
  ggsave(., filename = file.path(here(), "output", "amera-partners.pdf"),
         width = 5.5, height = 2.5, units = "in", device = cairo_pdf) %>% 
  ggsave(., filename = file.path(here(), "output", "amera-partners.png"),
         width = 5.5, height = 2.5, units = "in", type = "cairo", dpi = 300)
```
